...
func TestWritere(t *testing.T) {
	...
	t.Run("Write game", func(t *testing.T) {
		// Act
		game := data.NewPlaystationStoreGame(random.String(10), "https://www.xbox.com/pl-pl/games/store/lego-batman-3-beyond-gotham-deluxe-edition/c4hfhz44z3r3", 21.37, 69.0)

		err := writer.Write(ctx, []data.PlaystationStoreGame{game})
		assert.NoError(t, err)
	})

	t.Run("Write empty array", func(t *testing.T) {

		err := writer.Write(ctx, []data.PlaystationStoreGame{})
		assert.Error(t, err)
	})
	...
	t.Run("Write twice game", func(t *testing.T) {
		// Act
		game := data.NewPlaystationStoreGame(random.String(10), "https://www.xbox.com/pl-pl/games/store/lego-batman-3-beyond-gotham-deluxe-edition/c4hfhz44z3r3", 21.37, 69.0)
		game2 := data.NewPlaystationStoreGame(random.String(10), "https://www.xbox.com/pl-pl/games/store/lego-batman-3-beyond-gotham-deluxe-edition/c4hfhz44z3r3", 21.37, 69.0)
		err := writer.Write(ctx, []data.PlaystationStoreGame{game})
		assert.NoError(t, err)

		err = writer.Write(ctx, []data.PlaystationStoreGame{game2})
		assert.NoError(t, err)
	})
}
